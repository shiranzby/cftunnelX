...
```powershell
go build -buildvcs=false -ldflags "-s -w -H=windowsgui" -o dist\cftunnelX-v4.9.exe .
```

Windows CLI：

```powershell
go build -buildvcs=false -ldflags "-s -w" -o dist\cftunnelX-v4.9-cli.exe .
```
...
| 文件 | 定位 | 行为 |
| --- | --- | --- |
| `dist/cftunnelX-v4.9.exe` | 浏览器 GUI 双击版 | 启动内置 Web 服务并用系统默认浏览器打开 |
| `desktop-client/build/bin/cftunnelX-desktop.exe` | Wails 桌面客户端 | 原生窗口嵌入 WebUI，并自动拉起同目录 CLI/Web 服务 |
| `dist/cftunnelX-v4.9-windows-portable.zip` | Windows portable 包 | 包含 GUI、CLI、Wails 桌面端、README、LICENSE、依赖文件、`config/` 与 `log/` |

## Docker

```bash
docker build -t cftunnelx:v4.9 .
docker compose up -d
```
